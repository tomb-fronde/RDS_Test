...
namespace NZPostOffice.RDSAdmin.Entity.Security
{
	...
	public class TownDddw : Entity<TownDddw>
	{
		...
		public static TownDddw NewTownDddw(  )
		{
			return Create();
		}

		public static TownDddw[] GetAllTownDddw(  )
		{
		...
		private void FetchEntity(  )
		{
			using ( DbConnection cn= DbConnectionFactory.RequestNextAvaliableSessionDbConnection("NZPO" ))
			{
				using (DbCommand cm = cn.CreateCommand())
				{
					...
					List<TownDddw> list = new List<TownDddw>();
					using (MDbDataReader dr = DBHelper.ExecuteReader(cm, pList))
					{
						while (dr.Read())
						{
							TownDddw instance = new TownDddw();
							instance.StoreFieldValues(dr, "towncity");
							instance.MarkOld();
							list.Add(instance);
						}
						dataList = new TownDddw[list.Count];
