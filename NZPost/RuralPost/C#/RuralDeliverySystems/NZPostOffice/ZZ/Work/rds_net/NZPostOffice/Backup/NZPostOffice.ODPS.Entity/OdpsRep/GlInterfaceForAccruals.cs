...
namespace NZPostOffice.ODPS.Entity.OdpsRep
{
    ...
    public class GlInterfaceForAccruals : Entity<GlInterfaceForAccruals>
    {
        ...
        public static GlInterfaceForAccruals NewGlInterfaceForAccruals(DateTime? ProcDate)
        {
            return Create(ProcDate);
        }

        public static GlInterfaceForAccruals[] GetAllGlInterfaceForAccruals(DateTime? ProcDate)
        {
        ...
        private void FetchEntity(DateTime? ProcDate)
        {
            using (DbConnection cn = DbConnectionFactory.RequestNextAvaliableSessionDbConnection("NZPO"))
            {
                using (DbCommand cm = cn.CreateCommand())
                {
                    ...
                    cm.CommandText = "odps.od_rps_gl_interface_for_accruals";
                    ParameterCollection pList = new ParameterCollection();
                    pList.Add(cm, "ProcDate", ProcDate);

                    List<GlInterfaceForAccruals> _list = new List<GlInterfaceForAccruals>();
                    using (MDbDataReader dr = DBHelper.ExecuteReader(cm, pList))
                    {
                        while (dr.Read())
                        {
                            GlInterfaceForAccruals instance = new GlInterfaceForAccruals();
