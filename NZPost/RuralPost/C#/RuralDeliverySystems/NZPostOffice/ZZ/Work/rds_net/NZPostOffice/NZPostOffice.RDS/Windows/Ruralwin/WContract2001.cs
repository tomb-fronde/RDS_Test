...
namespace NZPostOffice.RDS.Windows.Ruralwin
{
    ...
    // Added user event StatusUpdatedEventArgs so WRenewal could inform WContract 
    //    when the route_frequency table had been modified (vehicle made active or 
    //    inactive)so WContract could update its Frequencies tab (specifically the 
    ...
    // Removed all references to dw_route_frequency, idw_frequencies, and tabpage_frequencies
    // and related functions and procedures.
    // - replaced with dw_route_frequency2, idw_frequencies2, and tabpage_frequencies2 etc.
    // DataControl DRouteFrequency and DRouteFrequency.designer retained as deleting them 
    // (probably) removes them from the archive.
    // 
    ...
    public class WContract2001 : WAncestorWindow
    {
        ...
        public URdsDw idw_artical_counts;
        ...
        public TabPage tabpage_article_counts;
        ...
        private Button cb_seqAddresses;
        private Button cb_showAll;
        ...
        public WContract2001()
        {
            ...
            this.cb_seqAddresses.Click += new System.EventHandler(this.cb_seq_clicked);
            ...
            this.dw_route_frequency2.PfcPostUpdate += new UserEventDelegate(dw_route_frequency2_pfc_postupdate);
        ...
        public override void pfc_postopen()
        {
            ...
            idw_frequencies2.of_set_deletepriv(false);
            idw_route_audit.of_set_deletepriv(false);
            idw_renewals.of_set_createpriv(false);
            idw_artical_counts.of_set_deletepriv(false);
            idw_artical_counts.of_set_createpriv(false);
...
*/
        ...
        private void InitializeComponent()
        {
            ...
            this.cb_seqAddresses = new System.Windows.Forms.Button();
            ...
            this.cb_showAll = new System.Windows.Forms.Button();
            ...
            this.tabpage_article_counts = new System.Windows.Forms.TabPage();
            ...
            this.tabpage_article_counts.SuspendLayout();
            ...
            this.tab_contract.Controls.Add(this.tabpage_article_counts);
            ...
            this.tabpage_customers.Controls.Add(this.cb_seqAddresses);
            ...
            // cb_seqAddresses
            // 
            this.cb_seqAddresses.Location = new System.Drawing.Point(429, 273);
            this.cb_seqAddresses.Name = "cb_seqAddresses";
            this.cb_seqAddresses.Size = new System.Drawing.Size(116, 23);
            this.cb_seqAddresses.TabIndex = 5;
            this.cb_seqAddresses.Text = "Sequence Addresses";
            this.cb_seqAddresses.UseVisualStyleBackColor = true;
            // 
            // dw_contract_address
            // 
            ...
            this.tabpage_frequencies2.Controls.Add(this.cb_showAll);
            ...
            this.tabpage_frequencies2.Text = "Frequencies";
            ...
            this.cb_showAll.TabIndex = 5;
            this.cb_showAll.Text = "Show &Active Only";
            this.cb_showAll.TextAlign = System.Drawing.ContentAlignment.TopCenter;
            ...
            // 
            // tabpage_article_counts
            // 
            this.tabpage_article_counts.BackColor = System.Drawing.SystemColors.ButtonFace;
            this.tabpage_article_counts.Controls.Add(this.dw_artical_counts);
            this.tabpage_article_counts.ForeColor = System.Drawing.SystemColors.WindowText;
            this.tabpage_article_counts.Location = new System.Drawing.Point(4, 40);
            this.tabpage_article_counts.Name = "tabpage_article_counts";
            this.tabpage_article_counts.Size = new System.Drawing.Size(552, 300);
            this.tabpage_article_counts.TabIndex = 7;
            this.tabpage_article_counts.Tag = "ComponentName=Article Count;";
            this.tabpage_article_counts.Text = "Article Count";
            this.tabpage_article_counts.Visible = false;
            // 
            // dw_artical_counts
            // 
            ...
            this.tabpage_article_counts.ResumeLayout(false);
        ...
        public virtual bool of_validate_frequencies2()
        {
            ...
            // Derived from of_validate_frequencies (the previous version)
            ...
            for (ll_Row = 0; ll_Row < ll_RowCount; ll_Row++)
            {
                ...
                // TJB Frequencies 2-Dec-2020 - removed; not sure how to access dwChild for sf_key.
                string sSfDescription = "";
                dwChild = new DDddwStandardFrequency();
                dwChild.BindingSource.DataSource = idw_frequencies2.GetChild("sf_key").BindingSource;
                sfKey_Row = dwChild.Find("sf_key", lSFKey);
                if (sfKey_Row >= 0)
                {
                    sSfDescription = dwChild.GetItem<DddwStandardFrequency>(sfKey_Row).SfDescription;
                }
                ...
                {
                    MessageBox.Show("Please select a frequency for row " + (ll_Row + 1).ToString()
                                    , this.Text
                    ...
                    && idw_frequencies2.GetItem<RouteFrequency2>(ll_Row).CalcDeldays == "NNNNNNN")
                {
                    ...
                    MessageBox.Show("Please select the days that frequency '" + sSfDescription + "'"
                                    + " is delivered"
                                    , this.Text
                                    , MessageBoxButtons.OK, MessageBoxIcon.Stop);
                    error_found = true;
                    ...
                    if (sfKey_Row >= 0)  // TJB Bugfix 4-Dec-2020 changed ">" to ">="
                    {
                        if (dwChild.GetItem<DddwStandardFrequency>(sfKey_Row).SfDaysWeek.GetValueOrDefault() != lNumDays)
                        {
                            MessageBox.Show("The delivery days selected does not equal the number of days \n"
                                            + "specified for frequency '" + sSfDescription + "'"
                                            , this.Text
                                            , MessageBoxButtons.OK, MessageBoxIcon.Information);
                            error_found = true;
                        }
        ...
        public virtual bool of_validate_articlecounts()
        {
            int ll_Row;
            int ll_RowCount;
            ll_RowCount = idw_artical_counts.RowCount;
            for (ll_Row = 0; ll_Row < ll_RowCount; ll_Row++)
            {
                if ((idw_artical_counts.GetItem<ContractArticalCounts>(ll_Row).ContractNo == null))
                {
                    idw_artical_counts.GetItem<ContractArticalCounts>(ll_Row).ContractNo = il_Contract_no;
                }
            }
            idw_artical_counts.AcceptText();
            return true;
        }
        ...
        // Update the route_frequencies table
        public virtual int of_update_route_frequencies2()
        {
            int nRows = idw_frequencies2.Save();
/*
            if (nRows > 0)
            {   // If successful, tell WRenewal via the ContractRFUpdatedEvent event
...
*/                
        ...
        public virtual int dw_renewals_pfc_predeleterow()
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    idw_artical_counts.Retrieve(new object[]{lcontract});
                }
            }
            return 1;
        }
        ...
        {   // TJB Frequencies Nov-2020  NEW
            // From dw_route_frequency_ue_enabledistance()
            // NOTE: This is misnamed; it disables changes to the Distance, not enables it
            ...
            if (ll_count != 1)
            {
                ...
                //MessageBox.Show("dw_route_frequency2 \n"
                //                + "NRow = " + nRow.ToString() + " NRows = " + nRows.ToString()
                //                , "WContract2001.dw_route_frequency2_ue_enabledistance");
                //dw_route_frequency.GetControlByName("grid"))).Columns["distance"].ReadOnly = true;
            }
        }

        public virtual void dw_route_frequency2_DoubleClick(object sender, EventArgs e)
        {   // TJB Frequencies Nov-2020  NEW
            // From dw_route_frequency_doubleclicked()
            ...
            NCriteria lnv_Criteria;
            NRdsMsg lnv_msg;
            int row = dw_route_frequency2.GetRow();
            if (row < 0)
            {
                return;
            }
            ...
            lnv_Criteria = new NCriteria();
            lnv_msg = new NRdsMsg();
            if (dw_route_frequency2.GetItem<RouteFrequency2>(row).RfActive == "N")
            {
            ...
            if (dw_route_frequency2.GetItem<RouteFrequency2>(row).IsNew
                || (dw_route_frequency2.GetItem<RouteFrequency2>(row).IsNew
                    && dw_route_frequency2.GetItem<RouteFrequency2>(row).IsDirty))
            {
            ...
            int nSFKey, childRow;
            string sSfDescription;
            nSFKey = idw_frequencies2.GetItem<RouteFrequency2>(row).SfKey.GetValueOrDefault();
            dwChild = new DDddwStandardFrequency();
            dwChild.BindingSource.DataSource = idw_frequencies2.GetChild("sf_key").BindingSource;
            childRow = dwChild.Find("sf_key", nSFKey);
            sSfDescription = dwChild.GetItem<DddwStandardFrequency>(childRow).SfDescription;

            lnv_Criteria.of_addcriteria("Contract_no", dw_route_frequency2.GetItem<RouteFrequency2>(row).ContractNo.GetValueOrDefault());
            lnv_Criteria.of_addcriteria("Sf_key", dw_route_frequency2.GetItem<RouteFrequency2>(row).SfKey.GetValueOrDefault());
            lnv_Criteria.of_addcriteria("Rf_delivery_days", dw_route_frequency2.GetItem<RouteFrequency2>(row).RfDeliveryDays);
            lnv_Criteria.of_addcriteria("Dw_route_freq", dw_route_frequency2);
            lnv_Criteria.of_addcriteria("Sf_description", sSfDescription);
            lnv_msg.of_addcriteria(lnv_Criteria);
            ls_Title = " (" + dw_route_frequency2.GetItem<RouteFrequency2>(row).ContractNo.GetValueOrDefault().ToString() + ") ";
        ...
        public virtual void dw_route_frequency2_clicked(object sender, EventArgs e)
        {   // TJB Frequencies Nov-2020  NEW
            // From dw_route_frequency_clicked()
        ...
        {
            //MessageBox.Show("dw_route_frequency2_PfcPreInsertRow");
            of_save_contract_no();
            return 1;
        }
...
*/
            ...
            RDSDataService obj = RDSDataService.GetLatestBenchmark(il_Contract_no, il_con_active_seq);
    ...
    public virtual void dw_route_audit_constructor()
        ...
        public virtual void dw_artical_counts_constructor()
        {
            ...
            //?inv_rowselect.of_SetStyle(0);
            idw_artical_counts = dw_artical_counts;
            //idw_artical_counts.DataObject.RetrieveEnd += new EventHandler(DataObject_RetrieveEnd);
        }
        ...
        public virtual void of_save_contract_no()
        {
            // TJB Frequencies Nov-2020 NEW
            // Place the contract number in the shared place to be 
            // retrieved by DDwwContractorVehicles (and perhaps others)

            StaticMessage.ContractNoParm = il_Contract_no;
        }
        #endregion
        ...
        public virtual void tab_contract_selectionchanging(object sender, EventArgs e)
        {
            ...
            if (ib_Opening)
            {
                ...
                idw_artical_counts.ResetUpdate();
...
*/            
            ...
            if (idw_frequencies2.DataObject.DeletedCount > 0 || idw_frequencies2.ModifiedCount() > 0)
            {
                ...
                if (ans == DialogResult.Yes)
                {
                    ...
                    ll_ret = of_update_route_frequencies2();
                    if (ll_ret < 0)
                        return;
                }
                else
...
//                idw_frequencies.URdsDw_Clicked(null, null);
            ...
            {
                idw_frequencies2.URdsDw_GetFocus(null, null);
...
//                idw_frequencies2.URdsDw_Clicked(null, null);
            ...
            {
                idw_artical_counts.URdsDw_GetFocus(null, null);

                if (idw_artical_counts.RowCount == 0)
                {
                    idw_artical_counts.Retrieve(new object[] { il_Contract_no });
                }
                idw_artical_counts.GetControlByName("st_contract").Text = idw_contract.GetItem<Contract>(0).ConTitle;
                idw_artical_counts.uf_settoolbar();
            }
        ...
        public virtual void dw_renewals_doubleclicked(object sender, EventArgs e)
        {
            ...
            if (!((StaticVariables.gnv_app.of_findwindow(ls_Title, "w_Renewal2001") != null)))
            {
                ...
                lw_Renewal2001.RenewalRFTableUpdated += new EventHandler<StatusUpdatedEventArgs>(Renewal_RFTableUpdated);
                lw_Renewal2001.Show();  
            }
        }
        ...
        void Renewal_RFTableUpdated(object sender, StatusUpdatedEventArgs e)
        {
...
/*
        // TJB  Frequencies & Vehicles 11-Feb-2021: New
        // User event used to "tell" the WRenewal2001 form that we've 
        // updated the route_frequency table. 
        // See NZPostOffice.RDS.Controls.StatusUpdatedEventArgs.cs
        public event EventHandler<ContractRFUpdatedEventArgs> ContractRFUpdated;

        // TJB  Frequencies & Vehicles 11-Feb-2021: New
        protected virtual void OnContractRFUpdated(ContractRFUpdatedEventArgs e)
        {
            if (ContractRFUpdated != null)
                ContractRFUpdated(this, e);
        }
*/
        #endregion
    }
