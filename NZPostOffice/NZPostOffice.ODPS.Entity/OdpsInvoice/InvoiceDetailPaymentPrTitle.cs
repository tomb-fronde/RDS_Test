...
namespace NZPostOffice.ODPS.Entity.OdpsInvoice
{
    ...
    public class InvoiceDetailPaymentPrTitle : Entity<InvoiceDetailPaymentPrTitle>
    {
        ...
        public static InvoiceDetailPaymentPrTitle NewInvoiceDetailPaymentPrTitle(string in_message)
        {
            return Create(in_message);
        }

        public static InvoiceDetailPaymentPrTitle[] GetAllInvoiceDetailPaymentPrTitle(string in_message)
        {
        ...
        private void FetchEntity(string in_message)
        {
            using (DbConnection cn = DbConnectionFactory.RequestNextAvaliableSessionDbConnection("NZPO"))
            {
                using (DbCommand cm = cn.CreateCommand())
                {
                    ...
                    List<InvoiceDetailPaymentPrTitle> _list = new List<InvoiceDetailPaymentPrTitle>();
                    using (MDbDataReader dr = DBHelper.ExecuteReader(cm, pList))
                    {
                        while (dr.Read())
                        {
                            InvoiceDetailPaymentPrTitle instance = new InvoiceDetailPaymentPrTitle();
