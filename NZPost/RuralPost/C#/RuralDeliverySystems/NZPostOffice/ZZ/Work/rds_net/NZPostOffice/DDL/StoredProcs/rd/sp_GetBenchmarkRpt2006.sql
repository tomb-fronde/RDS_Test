if exists (select 1 from sysobjects where name = 'sp_GetBenchmarkRpt2006' and type = 'P')
begin
    print 'Dropping procedure sp_GetBenchmarkRpt2006'
    drop procedure sp_GetBenchmarkRpt2006
end
go
print 'Creating procedure sp_GetBenchmarkRpt2006'
go

/****** Object:  StoredProcedure [rd].[sp_GetBenchmarkRpt2006]    Script Date: 08/05/2008 10:19:12 ******/
SET ANSI_NULLS ON
GO
...
--
-- TJB  RD7_0005 Aug 2008 - Apply SR4721 changes
-- TJB  SR4721 July 2008 
--      - Moved dExpiryDate to inSequence contract from inSequence+1's 
...
from
    contract_renewals as cr
        left outer join non_vehicle_override_rate as nvor 
                on cr.contract_no = nvor.contract_no     --!and cr.contract_seq_number = nvor.contract_seq_number
...
if exists (select 1 from sysobjects where name = 'sp_GetBenchmarkRpt2006' and type = 'P')
    print 'Procedure sp_GetBenchmarkRpt2006 created'
else
    print '*** Error *** Procedure sp_GetBenchmarkRpt2006 NOT created!'
go
