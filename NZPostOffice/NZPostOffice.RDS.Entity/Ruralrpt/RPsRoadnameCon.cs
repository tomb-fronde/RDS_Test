...
namespace NZPostOffice.RDS.Entity.Ruralrpt
{
	...
	public class PsRoadnameCon : Entity<PsRoadnameCon>
	{
		...
		public static PsRoadnameCon NewPsRoadnameCon( int? con_id )
		{
			return Create(con_id);
		}

		public static PsRoadnameCon[] GetAllPsRoadnameCon( int? con_id )
		{
		...
		private void FetchEntity( int? con_id )
		{
			using ( DbConnection cn= DbConnectionFactory.RequestNextAvaliableSessionDbConnection( "NZPO"))
			{
                using (DbCommand cm = cn.CreateCommand())
                {
                    ...
                    cm.CommandText = "rd.sp_getroads_con";
                    ParameterCollection pList = new ParameterCollection();
                    pList.Add(cm, "@contract_id", con_id);

                    List<PsRoadnameCon> _list = new List<PsRoadnameCon>();
                    using (MDbDataReader dr = DBHelper.ExecuteReader(cm, pList))
                    {
                        while (dr.Read())
                        {
                            PsRoadnameCon instance = new PsRoadnameCon();
