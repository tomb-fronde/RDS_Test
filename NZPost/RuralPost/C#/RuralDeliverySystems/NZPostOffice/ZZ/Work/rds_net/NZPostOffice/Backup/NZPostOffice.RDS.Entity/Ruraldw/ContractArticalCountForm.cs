...
namespace NZPostOffice.RDS.Entity.Ruraldw
{
    ...
    public class ContractArticalCountForm : Entity<ContractArticalCountForm>
    {
        ...
        //if(isnull(contract_seq_number), '<;none>;',string(contract_no) + '/' + string(contract_seq_number, '##'))
        public virtual string Compute1
        {
            get
            {
                CanReadProperty("Compute1", true);
                return _contract_seq_number == null ? "<none>" : _contract_no.GetValueOrDefault().ToString() + "/" + _contract_seq_number.GetValueOrDefault().ToString("##");
            }
        }
        ...
        public virtual int? Week1del
        {
            get
            {
                ...
                return (_ac_w1_medium_letters == null ? 0 : _ac_w1_medium_letters) + (_ac_w1_other_envelopes == null ? 0 : _ac_w1_other_envelopes) + (_ac_w1_small_parcels == null ? 0 : _ac_w1_small_parcels);
            }
        }
        ...
        public static ContractArticalCountForm NewContractArticalCountForm(int? in_Contract)
        {
            return Create(in_Contract);
        }

        public static ContractArticalCountForm[] GetAllContractArticalCountForm(int? in_Contract)
        {
        ...
        private void FetchEntity(int? in_Contract)
        {
            using (DbConnection cn = DbConnectionFactory.RequestNextAvaliableSessionDbConnection("NZPO"))
            {
                using (DbCommand cm = cn.CreateCommand())
                {
                    ...
                    cm.CommandText = "sp_GetContractArtCnts";
                    List<ContractArticalCountForm> _list = new List<ContractArticalCountForm>();
                    using (MDbDataReader dr = DBHelper.ExecuteReader(cm, pList))
                    {
                        while (dr.Read())
                        {
                            ContractArticalCountForm instance = new ContractArticalCountForm();
