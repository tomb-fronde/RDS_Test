...
namespace NZPostOffice.RDS.Entity.Ruraldw
{
    ...
    public class PieceRateImportDiffRateReport : Entity<PieceRateImportDiffRateReport>
    {
        ...
        //?round(prd_quantity  *  prd_rd_cost,3)
        public virtual decimal? Totalcost
        {
            get
            {
                CanReadProperty("Totalcost", true);
                return Math.Round((_prd_quantity * _prd_rd_cost).GetValueOrDefault(), 3);
            }
        }

        public virtual decimal? RETotalcost
        {
            get
            {
                return Math.Round((_prd_quantity * _prd_rd_cost).GetValueOrDefault(), 3);
        ...
        public static PieceRateImportDiffRateReport NewPieceRateImportDiffRateReport()
        {
            return Create();
        }

        public static PieceRateImportDiffRateReport[] GetAllPieceRateImportDiffRateReport()
        {
            return Fetch().list;
        }
        #endregion
        ...
        //            List<PieceRateImportDiffRateReport> _list = new List<PieceRateImportDiffRateReport>();
        //            using (MDbDataReader dr = DBHelper.ExecuteReader(cm, pList))
        //            {
        //                while (dr.Read())
        //                {
        //                    PieceRateImportDiffRateReport instance = new PieceRateImportDiffRateReport();
        //                    instance.MarkOld();
        //                    _list.Add(instance);
        //                }
        //                list = _list.ToArray();
