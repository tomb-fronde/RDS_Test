...
namespace NZPostOffice.ODPS.Entity.OdpsLib
{
    ...
    public class ShellImport : Entity<ShellImport>
    {
        ...
        {
            return  string.Format( "{0}", _column1);
        }

        #endregion

        #region Factory Methods
        public static ShellImport NewShellImport()
        {
            return Create();
        }

        public static ShellImport[] GetAllShellImport()
        {
            return Fetch().list;
        }
        #endregion
        ...
        private void FetchEntity()
        {
            using (DbConnection cn = DbConnectionFactory.RequestNextAvaliableSessionDbConnection("NZPO"))
            {
                using (DbCommand cm = cn.CreateCommand())
                {
                    ...
                    List<ShellImport> _list = new List<ShellImport>();
                    using (MDbDataReader dr = DBHelper.ExecuteReader(cm, pList))
                    {
                        while (dr.Read())
                        {
                            ShellImport instance = new ShellImport();
                            instance.MarkOld();
