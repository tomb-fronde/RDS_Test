...
namespace NZPostOffice.RDS.Windows.Ruralwin2
{
    ...
    public partial class WOverrideRates : WMaster
    {
        #region Define
        ...
        public WOverrideRates()
        {
            this.InitializeComponent();
            this.ShowInTaskbar = false;

            dw_vehicle_override_rates.DataObject = new DVehicleOverrideRates();
            dw_non_vehicle_rates.DataObject = new DNonVehicleOverrideRates();
            dw_other_rates.DataObject = new DOtherOverrideRates();

            dw_vehicle_override_rates.Constructor += new NZPostOffice.RDS.Controls.UserEventDelegate(dw_vehicle_constructor);
            dw_vehicle_override_rates.PfcUpdate += new NZPostOffice.RDS.Controls.UserEventDelegate1(dw_vehicle_override_rates_pfc_update);
            dw_vehicle_override_rates.PfcValidation += new UserEventDelegate1(dw_vehicle_override_rates_pfc_validation);
            dw_vehicle_override_rates.DataObject.GotFocus += new EventHandler(dw_vehicle_override_rates_getfocus);
            ((DVehicleOverrideRates)dw_vehicle_override_rates.DataObject).TextBoxLostFocus += new EventHandler(dw_vehicle_override_rates_itemchanged);
            //dw_renewal_freq_adjust.PfcDeleteRow += new UserEventDelegate(dw_renewal_freq_adjust_pfc_deleterow);
            dw_vehicle_override_rates.PfcPreDeleteRow += new UserEventDelegate1(dw_vehicle_override_rates_pfc_predeleterow);
            dw_vehicle_override_rates.PfcDeleteRow += new NZPostOffice.RDS.Controls.UserEventDelegate(dw_vehicle_override_rates_pfc_deleterow);
        ...
        public override void pfc_postopen()
        {
            ...
            iVehicleNo = StaticVariables.gnv_app.of_get_parameters().intparm;

            if (iVehicleNo == null || iVehicleNo <= 0)
            {
                MessageBox.Show("Please select a vehicle", this.Text);
                this.Close();
            }
            sVehicleName = RDSDataService.GetVehicleName(iVehicleNo, ref SQLCode, ref SQLErrText);
            ls_heading = this.Text
                         + " " + il_contract.ToString() + "/" + il_sequence.ToString()
                         + "   " + sVehicleName;
                this.Text = ls_heading;

            // Retrieve rates
            // TJB  Frequencies & Vehicles  22-Jan-2021
            // Added vehicle_number to retrieve parameters
            idw_vehicleoverriderates.Retrieve(new object[] { il_contract, il_sequence, iVehicleNo });
            idw_nonvehicleoverriderates.Retrieve(new object[] { il_contract, il_sequence, iVehicleNo });
            ...
            // 	tab_override_rates.tabpage_vehicle_rates.dw_vehicle_override_rates.of_ProtectColumns()
            ...
            // End If
            il_economiclife = of_getremainingeconomiclife(iVehicleNo);
        ...
        public virtual int? of_getremainingeconomiclife(int? iVehicleNo)
        {
            int? ll_ecolife = null;
            // Get remaining economic life for the vehicle
            RDSDataService dataService = RDSDataService.GetVehicleLifeCode(il_sequence, il_contract, iVehicleNo);
        ...
        {
            idw_vehicleoverriderates = dw_vehicle_override_rates;
        }
        ...
        public virtual int dw_vehicle_override_rates_pfc_validation()
        {
            ...
            dw_vehicle_override_rates.AcceptText();
            ld_effective_date = dw_vehicle_override_rates.GetItem<VehicleOverrideRates>(0).VorEffectiveDate;
            if (il_inserted == 1)
            {
        ...
        ************************************************************************/
        ...
        public virtual void dw_vehicle_override_rates_getfocus(object sender, EventArgs e)
        {
            dw_vehicle_override_rates.URdsDw_GetFocus(sender, e);
            // If the datawindow retrieves a record make the datawindow readonly
            if (dw_vehicle_override_rates.RowCount > 0 
                && il_inserted != 1 
                && dw_vehicle_override_rates.GetItem<VehicleOverrideRates>(0).VorEffectiveDate != null)
            {
                foreach (Control var in dw_vehicle_override_rates.DataObject.Controls)
                {
                    if (var is TextBox)
                    {
        ...
        public virtual void dw_vehicle_override_rates_itemchanged(object sender, EventArgs e)
        {
            dw_vehicle_override_rates.URdsDw_Itemchanged(sender, e);
            // Set the contract_no and the contract_seq_no
            // tab_override_rates.tabpage_vehicle_rates.dw_vehicle_override_rates.SetItem(1, "Contract_no", il_contract)
            // tab_override_rates.tabpage_vehicle_rates.dw_vehicle_override_rates.SetItem(1, "Contract_seq_number", il_sequence)
        ...
        public virtual void of_create_newrates()
        {
            /***********************************************************************
            ...
            ********************************************************************** */
            ...
            if (answer == DialogResult.Yes)
            {
                ...
                nRow1 = nRow;
                // TJB Jan-2021: Update the vehicle_number if we're adding overrides for a new vehicle
                int? nVehNo = idw_vehicleoverriderates.GetItem<VehicleOverrideRates>(0).VehicleNumber;
                if (nVehNo == null || nVehNo != iVehicleNo)
                {
                    idw_vehicleoverriderates.GetItem<VehicleOverrideRates>(0).VehicleNumber = iVehicleNo;
                }
