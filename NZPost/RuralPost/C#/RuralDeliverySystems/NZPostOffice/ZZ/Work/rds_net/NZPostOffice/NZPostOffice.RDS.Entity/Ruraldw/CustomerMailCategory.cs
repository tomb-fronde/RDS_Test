...
namespace NZPostOffice.RDS.Entity.Ruraldw
{
	...
	public class CustomerMailCategory : Entity<CustomerMailCategory>
	{
		...
		public static CustomerMailCategory NewCustomerMailCategory( int? in_Cust_Id )
		{
			return Create(in_Cust_Id);
		}

		public static CustomerMailCategory[] GetAllCustomerMailCategory( int? in_Cust_Id )
		{
		...
		private void FetchEntity( int? in_Cust_Id )
		{
			using ( DbConnection cn= DbConnectionFactory.RequestNextAvaliableSessionDbConnection( "NZPO"))
			{
				using (DbCommand cm = cn.CreateCommand())
				{
                    ...
                    cm.CommandText = "sp_GetCustMailCat";
					List<CustomerMailCategory> _list = new List<CustomerMailCategory>();
					using (MDbDataReader dr = DBHelper.ExecuteReader(cm, pList))
					{
						while (dr.Read())
						{
							CustomerMailCategory instance = new CustomerMailCategory();
