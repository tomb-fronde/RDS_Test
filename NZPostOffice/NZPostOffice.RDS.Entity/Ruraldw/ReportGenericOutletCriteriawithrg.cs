...
namespace NZPostOffice.RDS.Entity.Ruraldw
{
    ...
    public class ReportGenericOutletCriteriawithrg : Entity<ReportGenericOutletCriteriawithrg>
    {
        ...
        public static ReportGenericOutletCriteriawithrg NewReportGenericOutletCriteriawithrg()
        {
            return Create();
        }

        public static ReportGenericOutletCriteriawithrg[] GetAllReportGenericOutletCriteriawithrg()
        {
            return Fetch().list;
        }
        #endregion
        ...
        private void FetchEntity()
        {
            using (DbConnection cn = DbConnectionFactory.RequestNextAvaliableSessionDbConnection("NZPO"))
            {
                using (DbCommand cm = cn.CreateCommand())
                {
                    ...
                    cm.CommandText = "  SELECT outlet.region_id,            outlet.outlet_id,            outlet.o_name,            0 outlet_picklist,            contract_type.ct_key,            renewal_group.rg_code,            standard_frequency.sf_key,            0 rg_code1,            0 rg_code2,            CAST(CAST(GETDATE() AS INTEGER) AS DATETIME) mail_count_date,            0 blankforms      FROM outlet,            contract_type,            renewal_group,            standard_frequency   ";

                    List<ReportGenericOutletCriteriawithrg> _list = new List<ReportGenericOutletCriteriawithrg>();
                    using (MDbDataReader dr = DBHelper.ExecuteReader(cm, pList))
                    {
                        while (dr.Read())
                        {
                            ReportGenericOutletCriteriawithrg instance = new ReportGenericOutletCriteriawithrg();
