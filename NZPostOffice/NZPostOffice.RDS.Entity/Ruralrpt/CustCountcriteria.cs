...
namespace NZPostOffice.RDS.Entity.Ruralrpt
{
	...
	public class CustCountcriteria : Entity<CustCountcriteria>
	{
		...
		public static CustCountcriteria NewCustCountcriteria(  )
		{
			return Create();
		}

		public static CustCountcriteria[] GetAllCustCountcriteria(  )
		{
			return Fetch().list;
		}
		#endregion
		...
		private void FetchEntity(  )
		{
			using ( DbConnection cn= DbConnectionFactory.RequestNextAvaliableSessionDbConnection( "NZPO"))
			{
				using (DbCommand cm = cn.CreateCommand())
				{
                    ...
                    cm.CommandText = "SELECT outlet.region_id, outlet.outlet_id, outlet.o_name, 0 outlet_picklist, contract_type.ct_key, renewal_group.rg_code, standard_frequency.sf_key, 0 rg_code1, 0 rg_code2, getdate() mail_count_date, 0 blankforms FROM outlet, contract_type, renewal_group, standard_frequency";
					ParameterCollection pList = new ParameterCollection();

					List<CustCountcriteria> _list = new List<CustCountcriteria>();
					using (MDbDataReader dr = DBHelper.ExecuteReader(cm, pList))
					{
						while (dr.Read())
						{
							CustCountcriteria instance = new CustCountcriteria();
