/****** Object:  StoredProcedure [rd].[sp_GetWhatIfCalc2005]    Script Date: 08/05/2008 10:21:33 ******/
SET ANSI_NULLS ON
GO
...
begin
  ...
  select 
    @nVolume2=contract_renewals.con_volume_at_renewal 
  from 
    contract_renewals 
  where
  ...
  select 
    @nVolume1avg=(artical_count.ac_w1_medium_letters+
    artical_count.ac_w1_other_envelopes+
    artical_count.ac_w1_small_parcels+
    artical_count.ac_w2_medium_letters+
    artical_count.ac_w2_other_envelopes+
  ...
  select 
    @nVolume2avg=(artical_count.ac_w1_medium_letters+
    artical_count.ac_w1_other_envelopes+
    artical_count.ac_w1_small_parcels+
    artical_count.ac_w2_medium_letters+
    artical_count.ac_w2_other_envelopes+
  ...
  select top 1
    ...
    @nVtKey=v.vt_key 
  from
    contract_renewals as cr left outer join non_vehicle_override_rate as nvor 
    ...
    vehicle_type as vt left outer join vehicle_rate as vr 
  ...
  or @inVolumeSource = 'average_article_count'
    ...
    from
      contract_renewals INNER JOIN route_frequency 
  ...
  select 
    @nCurrentPayment=(contract_renewals.con_renewal_payment_value),
  ...
  select 
    @nSumAdjustments=sum(fd_amount_to_pay) 
  from 
  ...
  BEGIN
    ...
    select @nRateofReturnCost=rd.GetRateReturn(@nNominalVehical,@nRateOfReturn,@nSalvageRatio)*(@nDeliveryDays/@nMaxDeliveryDays)
 ...
 --Calculate the benchmark
  ...
  --get standard rates 
  select 
    @vrr_nominal_vehical_value=vehicle_rate.vr_nominal_vehicle_value,
 ...
 from
    ...
    contract_vehical,
    vehicle ,
    vehicle_type left outer join vehicle_rate 
 ...
 where
  ...
  select 
    ...
    @nACCAmount,
    @nVtKey,
    @nReliefCost                  as nReliefCost,          -- Added TJB SR4596
    @nProcessingWageRate          as nProcessingWageRate,  -- TJB SR4661
    @vrr_proc_wage_rate           as vrr_proc_wage_rate,
    @nReliefWeeks                 as nReliefWeeks          -- TJB SR4703
end
GO
