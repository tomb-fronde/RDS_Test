...
namespace NZPostOffice.RDS.Entity.Ruralrpt
{
    ...
    public class CustInterestStat : Entity<CustInterestStat>
    {
        ...
        public virtual int? REInterestcount
        {
            get
            {
                return _interestcount;
        ...
        public static CustInterestStat NewCustInterestStat(int? inRegion, int? inOutlet, int? inContractType, int? inPrivacy)
        {
            return Create(inRegion, inOutlet, inContractType, inPrivacy);
        }

        public static CustInterestStat[] GetAllCustInterestStat(int? inRegion, int? inOutlet, int? inContractType, int? inPrivacy)
        {
        ...
        private void FetchEntity(int? inRegion, int? inOutlet, int? inContractType, int? inPrivacy)
        {
            using (DbConnection cn = DbConnectionFactory.RequestNextAvaliableSessionDbConnection("NZPO"))
            {
                using (DbCommand cm = cn.CreateCommand())
                {
                    ...
                    cm.CommandText = "rd.sp_GetCustinterestStatV2";
                    ...
                    List<CustInterestStat> _list = new List<CustInterestStat>();
                    using (MDbDataReader dr = DBHelper.ExecuteReader(cm, pList))
                    {
                        while (dr.Read())
                        {
                            CustInterestStat instance = new CustInterestStat();
