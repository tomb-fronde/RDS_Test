...
namespace NZPostOffice.RDS.Entity.Ruralrpt
{
	...
	public class VehExpRenewReport : Entity<VehExpRenewReport>
	{
		...
		public static VehExpRenewReport NewVehExpRenewReport( int? renew_id )
		{
			return Create(renew_id);
		}

		public static VehExpRenewReport[] GetAllVehExpRenewReport( int? renew_id )
		{
			return Fetch(renew_id, false).list;
		}

        public static VehExpRenewReport[] GetAllVehExpiryReport(int? renew_id)
        {
            return Fetch(renew_id, true).list;
        }
		#endregion

		#region Data Access
		[ServerMethod]
		private void FetchEntity( int? renew_id, bool expiryRecords)
		{
			using ( DbConnection cn= DbConnectionFactory.RequestNextAvaliableSessionDbConnection( "NZPO"))
			{
				using (DbCommand cm = cn.CreateCommand())
				{
                    ...
                    if (!expiryRecords)
                    {
                        cm.CommandText = "rd.sp_v_exp_renew";
                        pList.Add(cm, "renew_id", renew_id);
                    }
                    else {
                        cm.CommandText = "rd.sp_v_exp_region";
                        pList.Add(cm, "reg_id", renew_id);
                    }
					
					

					List<VehExpRenewReport> _list = new List<VehExpRenewReport>();
					using (MDbDataReader dr = DBHelper.ExecuteReader(cm, pList))
					{
						while (dr.Read())
						{
							VehExpRenewReport instance = new VehExpRenewReport();
