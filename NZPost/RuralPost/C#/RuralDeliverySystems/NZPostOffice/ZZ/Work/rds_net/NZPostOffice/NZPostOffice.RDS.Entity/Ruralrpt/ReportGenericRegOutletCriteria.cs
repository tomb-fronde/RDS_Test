...
namespace NZPostOffice.RDS.Entity.Ruralrpt
{
    ...
    [MapInfo("checked1", "_checked1", "outlet")]
    [MapInfo("checked2", "_checked2", "outlet")]
    [System.Serializable()]

    public class ReportGenericRegOutletCriteria : Entity<ReportGenericRegOutletCriteria>
    {
        ...
        [DBField()]
        private string _checked1;

        [DBField()]
        private string _checked2;
        ...
        public virtual bool? Checked1// string Checked1
        {
            get
            {
                CanReadProperty(true);
                return GetBoolean<string>(_checked1, "Y", "N");

            }
            set
            {
                CanWriteProperty(true);
                SetFromBoolean<string>(ref _checked1, value, "Y", "N");

            }

            //get
            //{
            //    CanReadProperty("Checked1", true);
            //    return _checked1;
            //}
            //set
            //{
            //    CanWriteProperty("Checked1", true);
            //    if ( _checked1 != value )
            //    {
            //        _checked1 = value;
            //        PropertyHasChanged();
        ...
        public virtual bool? Checked2// string Checked2
        {
            get
            {
                CanReadProperty(true);
                return GetBoolean<string>(_checked2, "Y", "N");

            }
            set
            {
                CanWriteProperty(true);
                SetFromBoolean<string>(ref _checked2, value, "Y", "N");

            }
            //get
            //{
            //    CanReadProperty("Checked2", true);
            //    return _checked2;
            //}
            //set
            //{
            //    CanWriteProperty("Checked2", true);
            //    if ( _checked2 != value )
            //    {
            //        _checked2 = value;
            //        PropertyHasChanged();
        ...
        public static ReportGenericRegOutletCriteria NewReportGenericRegOutletCriteria()
        {
            return Create();
        }

        public static ReportGenericRegOutletCriteria[] GetAllReportGenericRegOutletCriteria()
        {
            return Fetch().list;
        }
        #endregion
        ...
        private void FetchEntity()
        {
            using (DbConnection cn = DbConnectionFactory.RequestNextAvaliableSessionDbConnection("NZPO"))
            {
                using (DbCommand cm = cn.CreateCommand())
                {
					...
					List<ReportGenericRegOutletCriteria> _list = new List<ReportGenericRegOutletCriteria>();
					using (MDbDataReader dr = DBHelper.ExecuteReader(cm, pList))
					{
						while (dr.Read())
						{
							ReportGenericRegOutletCriteria instance = new ReportGenericRegOutletCriteria();
                            ...
                            instance._rg_code1 = GetValueFromReader<int?>(dr,7);
                            instance._rg_code2 = GetValueFromReader<int?>(dr,8);
                            instance._mail_count_date = GetValueFromReader<DateTime?>(dr,9);
                            instance._blankforms = GetValueFromReader<int?>(dr,10);
                            instance._checked1 = GetValueFromReader<string>(dr,11);
                            instance._checked2 = GetValueFromReader<string>(dr,12);
							instance.MarkOld();
