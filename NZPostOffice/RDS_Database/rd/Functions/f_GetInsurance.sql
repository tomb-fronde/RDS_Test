...
CREATE function [rd].[f_GetInsurance](
	@in_contractNo int,
	@inSequenceNo int,
	@inEffDate datetime)
returns int
  ...
  select top 1 
         @vn_insurance_baserate = 
                 isNull(vehicle_override_rate.vor_vehicle_insurance_premium,
                          (isNull(non_vehicle_rate.nvr_vehicle_insurance_base_premium,0)
                           + isNull(vehicle_rate.vr_vehicle_value_insurance_pct/100,0)
                                * isNull(vehicle_override_rate.vor_nominal_vehicle_value,
                                          isNull(vehicle_rate.vr_nominal_vehicle_value,0))))
    from vehicle_type left outer join vehicle_rate 
              on vehicle_type.vt_key = vehicle_rate.vt_key,
         contract left outer join non_vehicle_rate 
              on contract.rg_code = non_vehicle_rate.rg_code,
         contract_renewals left outer join vehicle_override_rate 
              on contract_renewals.contract_no = vehicle_override_rate.contract_no 
              and contract_renewals.contract_seq_number = vehicle_override_rate.contract_seq_number,
         contract_vehical,
         vehicle
