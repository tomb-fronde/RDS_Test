/*
 select [rd].[BenchmarkCalc2021]( 50216, 3)
 select [rd].[BenchmarkCalc2005]( 50216, 3)
 select [rd].[BenchmarkCalc2021]( 5109, 24)
 select [rd].[BenchmarkCalc2005]( 5109, 24)
*/
CREATE FUNCTION [rd].[BenchmarkCalc2021](
	@inContract int,
	@inSequence int)
RETURNS real
...
-- Derived from BenchmarkCalc2005
-- [Originally there could be only one vehicle per contract, now there ]
-- [can be more than one, with different vehicles associated with      ]
-- [different frequencies.                                             ]
-- Change BM calculation to calculate separate costs for each vehicle
-- used for a contract's frequency, summing each to give a cost for
-- the contract.
-- Simplified method of determining @vTempint (whether RUC needs
-- to be included). Method previously used in BenchmarkCalcVeh2005.
-- Moved retreival of non_vehicle override rates into cursor loop
-- and included their vehicle selection.
-- [23-Feb-2021] Changed f_GetConVehicleMaxDelDays to f_GetConVehicleDelDays
-- [3-Mar-2021] Added rf_active = 'Y' to queries for RouteDistance and nDeliveryHours etc
-- [21-Jul-2021] Match BenchmarkRpt; cosmetic changes for clarity
-- [23-Jul-2021] Removed @SumFdDistance from Route distance calc 
-- [21-Mar-2022] 'Fixed' mishandling of some non-vehicle costs
--     that were to have been treated as per-vehicle costs. 
/*******************************************************
 * This contains a mishmash of real and decimal values *
 * that shopuld be rationalised to decimal with the BM *
 * returned as a decimal.  - TJB 21Mar2022             *
 *******************************************************/
