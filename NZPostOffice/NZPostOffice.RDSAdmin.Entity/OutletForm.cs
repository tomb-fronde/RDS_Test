...
namespace NZPostOffice.RDSAdmin.Entity.Security
{
	...
	public class OutletForm : Entity<OutletForm>
	{
		...
		public static OutletForm NewOutletForm( int? al_outlet_id )
		{
			return Create(al_outlet_id);
		}

		public static OutletForm[] GetAllOutletForm( int? al_outlet_id )
		{
			return Fetch(al_outlet_id).dataList;
		}
		#endregion

		#region Data Access
		[ServerMethod]
		private void FetchEntity( int? al_outlet_id )
		{
			using ( DbConnection cn= DbConnectionFactory.RequestNextAvaliableSessionDbConnection("NZPO" ))
			{
				using (DbCommand cm = cn.CreateCommand())
				{
					...
					pList.Add(cm, "al_outlet_id", al_outlet_id);
					GenerateSelectCommandText(cm, "outlet");
                    cm.CommandText += " where outlet_id = @al_outlet_id";

					List<OutletForm> list = new List<OutletForm>();
					using (MDbDataReader dr = DBHelper.ExecuteReader(cm, pList))
					{
						while (dr.Read())
						{
							OutletForm instance = new OutletForm();
							instance.StoreFieldValues(dr, "outlet");
							instance.MarkOld();
							list.Add(instance);
						}
						dataList = new OutletForm[list.Count];
		...
		private void UpdateEntity()
		{
			using ( DbConnection cn = DbConnectionFactory.RequestNextAvaliableSessionDbConnection( "NZPO"))
			{
				...
				cm.CommandType = CommandType.Text;
					ParameterCollection pList = new ParameterCollection();
                cm.CommandText = "UPDATE outlet SET ot_code = @ot_code, o_name = @o_name, o_address = @o_address, o_telephone = @o_telephone, o_fax = @o_fax, o_manager = @o_manager, o_responsibility_code = @o_responsibility_code, o_phy_address = @o_phy_address WHERE outlet_id = @outlet_id";
                pList.Add(cm, "ot_code", _ot_code);
                pList.Add(cm, "o_name", _o_name);
                pList.Add(cm, "o_address", _o_address);
                pList.Add(cm, "o_telephone", _o_telephone);
                pList.Add(cm, "o_fax", _o_fax);
                pList.Add(cm, "o_manager", _o_manager);
                pList.Add(cm, "o_responsibility_code", _o_responsibility_code);
                pList.Add(cm, "o_phy_address", _o_phy_address);
