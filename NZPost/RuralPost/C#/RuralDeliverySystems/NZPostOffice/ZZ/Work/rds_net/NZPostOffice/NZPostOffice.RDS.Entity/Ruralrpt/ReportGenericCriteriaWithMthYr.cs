...
namespace NZPostOffice.RDS.Entity.Ruralrpt
{
    ...
    // Adapted from ReportGenericCriteriaWithMonth
    // Used with ScheduleB reports
    ...
    [MapInfo("mth_yr", "_mth_yr","")]
    [System.Serializable()]

    public class ReportGenericCriteriaWithMthYr : Entity<ReportGenericCriteriaWithMthYr>
    {
        DateTime dToday;
        string sMthYr;
        ...
        public virtual string MthYr
        {
            get
            {
                CanReadProperty("MthYr", true);
                return _mth_yr;
            }
            set
            {
                CanWriteProperty("MthYr", true);
                if (_mth_yr != value)
                {
                    _mth_yr = value;
                    PropertyHasChanged();
        ...
        public static ReportGenericCriteriaWithMthYr NewReportGenericCriteriaWithMthYr()
        {
            return Create();
        }

        public static ReportGenericCriteriaWithMthYr[] GetAllReportGenericCriteriaWithMthYr()
        {
            return Fetch().list;
        }
        #endregion
        ...
        private void FetchEntity()
        {
            using (DbConnection cn = DbConnectionFactory.RequestNextAvaliableSessionDbConnection("NZPO"))
            {
                ...
                {
                    sMthYr = DateTime.Now.Month.ToString("D2") + DateTime.Now.Year.ToString("D2");
                    ...
                    List<ReportGenericCriteriaWithMthYr> _list = new List<ReportGenericCriteriaWithMthYr>();
                    using (MDbDataReader dr = DBHelper.ExecuteReader(cm, pList))
                    {
                        while (dr.Read())
                        {
                            ReportGenericCriteriaWithMthYr instance = new ReportGenericCriteriaWithMthYr();
                            ...
                            //instance._mth_yr = GetValueFromReader<DateTime?>(dr, 7);
                            instance._mth_yr = sMthYr;
                            instance.MarkOld();
