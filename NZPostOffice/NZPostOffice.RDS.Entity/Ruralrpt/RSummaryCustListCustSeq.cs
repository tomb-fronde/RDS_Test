...
namespace NZPostOffice.RDS.Entity.Ruralrpt
{
	...
	public class SummaryCustListCustSeq : Entity<SummaryCustListCustSeq>
	{
		...
		//?	Trim(if(isnull( cust_title ), '', cust_title) + ' ' + if(isnull(  cust_initials  ), '',  cust_initials ) +' '+ if(isnull(cust_surname_company) or cust_surname_company='ZZZZZ', '',   cust_surname_company ))
		...
		public static SummaryCustListCustSeq NewSummaryCustListCustSeq( int? in_contract_no, int? in_sf_key, string in_rd_del_days, string in_sortorder )
		{
			return Create(in_contract_no, in_sf_key, in_rd_del_days, in_sortorder);
		}

		public static SummaryCustListCustSeq[] GetAllSummaryCustListCustSeq( int? in_contract_no, int? in_sf_key, string in_rd_del_days, string in_sortorder )
		{
		...
		private void FetchEntity( int? in_contract_no, int? in_sf_key, string in_rd_del_days, string in_sortorder )
		{
			using ( DbConnection cn= DbConnectionFactory.RequestNextAvaliableSessionDbConnection( "NZPO"))
			{
				using (DbCommand cm = cn.CreateCommand())
				{
                    ...
                    cm.CommandText = "rd.sp_summary_cust_list_cust";
					...
					List<SummaryCustListCustSeq> _list = new List<SummaryCustListCustSeq>();
					using (MDbDataReader dr = DBHelper.ExecuteReader(cm, pList))
					{
						while (dr.Read())
						{
							SummaryCustListCustSeq instance = new SummaryCustListCustSeq();
