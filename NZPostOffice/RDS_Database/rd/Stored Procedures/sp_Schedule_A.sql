CREATE procedure [rd].[sp_Schedule_A](
	@inContract int,
	@inSequence int)
--
-- TJB  RPCR022  May 2011
-- Add Base office address to returned values
--
-- TJB  Hack     Aug 2006
...
begin
  ...
  select contractor.c_surname_company,
         ...
         outlet_combined=rd.strcat((case when outlet.o_name = 'Non-RD Dummy' then 'n.a.' else outlet.o_name end),
                                   (case when outlet_type.ot_outlet_type <> 'Other' then ' '+outlet_type.ot_outlet_type else '' end)),
         contract.contract_no,
         contract.con_title,
         lodgement=(select rd.strcat((case when outlet1.o_name = 'Non-RD Dummy' then 'n.a.' else outlet1.o_name end),
                                     (case when outlet_type1.ot_outlet_type <> 'Other' then ' '+outlet_type1.ot_outlet_type else '' end)) 
                      from outlet as outlet1,
                           outlet_type as outlet_type1 
                     where outlet_type1.ot_code = outlet1.ot_code and
         ...
         contractor.c_title,
         rgn_rcm_manager=region.rgn_rcm_manager
                         + case when rgn_telephone is null then '' else char(10)+'Telephone: '+rgn_telephone end
         ...
         contract.rg_code,
         oaddress = (case when outlet.o_phy_address is not null then outlet.o_phy_address else outlet.o_address end)
    from
         contract_renewals,
         contractor,
         outlet,
