...
namespace NZPostOffice.ODPS.Entity.OdpsRep
{
    ...
    public class AuditLogAddresschange : Entity<AuditLogAddresschange>
    {
        ...
        public static AuditLogAddresschange NewAuditLogAddresschange(DateTime? fromdate, DateTime? todate)
        {
            return Create(fromdate, todate);
        }

        public static AuditLogAddresschange[] GetAllAuditLogAddresschange(DateTime? fromdate, DateTime? todate)
        {
        ...
        private void FetchEntity(DateTime? fromdate, DateTime? todate)
        {
            using (DbConnection cn = DbConnectionFactory.RequestNextAvaliableSessionDbConnection("NZPO"))
            {
                using (DbCommand cm = cn.CreateCommand())
                {
                    ...
                    cm.CommandText = "rd.sp_rd_auditlog_addresschange";
                    ParameterCollection pList = new ParameterCollection();
                    pList.Add(cm, "fromdate", fromdate);
                    pList.Add(cm, "todate", todate);

                    List<AuditLogAddresschange> _list = new List<AuditLogAddresschange>();
                    using (MDbDataReader dr = DBHelper.ExecuteReader(cm, pList))
                    {
                        while (dr.Read())
                        {
                            AuditLogAddresschange instance = new AuditLogAddresschange();
