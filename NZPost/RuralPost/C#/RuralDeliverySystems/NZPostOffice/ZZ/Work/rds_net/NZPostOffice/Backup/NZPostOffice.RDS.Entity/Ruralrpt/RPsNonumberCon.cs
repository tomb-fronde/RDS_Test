...
namespace NZPostOffice.RDS.Entity.Ruralrpt
{
	...
	[MapInfo("cust_surname", "_cust_surname", "")]
	...
	public class PsNonumberCon : Entity<PsNonumberCon>
	{
		...
		public static PsNonumberCon NewPsNonumberCon( int? con_id )
		{
			return Create(con_id);
		}

		public static PsNonumberCon[] GetAllPsNonumberCon( int? con_id )
		{
		...
		private void FetchEntity( int? con_id )
		{
			using ( DbConnection cn= DbConnectionFactory.RequestNextAvaliableSessionDbConnection( "NZPO"))
			{
				using (DbCommand cm = cn.CreateCommand())
				{
                    ...
                    cm.CommandText = "rd.sp_nonumber_con";
					ParameterCollection pList = new ParameterCollection();
					pList.Add(cm, "con_id", con_id);

					List<PsNonumberCon> _list = new List<PsNonumberCon>();
					using (MDbDataReader dr = DBHelper.ExecuteReader(cm, pList))
					{
						while (dr.Read())
						{
							PsNonumberCon instance = new PsNonumberCon();
