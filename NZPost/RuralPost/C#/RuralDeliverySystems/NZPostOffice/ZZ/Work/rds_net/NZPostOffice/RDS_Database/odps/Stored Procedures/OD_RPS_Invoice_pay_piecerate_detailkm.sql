CREATE procedure [odps].[OD_RPS_Invoice_pay_piecerate_detailkm](
	@invoiceid int,
	@contractno int,
	@contractorno int,
...
BEGIN
  ...
  begin
                  ...
                  (select max(cr2.contract_seq_number) 
                    ...
                    where cr2.contract_no = @contractno and
                         ...
                         (contract.con_date_terminated is null 
                          ...
                          or contract.con_date_terminated between @payperiod_start and @payperiod_end) 
      ...
      select t_invoice_piecerates.invoice_id,
             ...
             t_invoice_piecerates.atype            
        from t_invoice_piecerates 
       where t_invoice_piecerates.invoice_id = @invoiceid and
             t_invoice_piecerates.prd_date between @v_contractor_start and @v_contractor_end 
       order by t_invoice_piecerates.prd_date asc,
                t_invoice_piecerates.prt_code asc
  end
  else    -- When there's only one contractor, do this
      select t_invoice_piecerates.invoice_id,
             t_invoice_piecerates.prd_date,
             t_invoice_piecerates.prt_code,
             t_invoice_piecerates.prd_quantity,
             ...
             t_invoice_piecerates.atype
        from t_invoice_piecerates 
       where t_invoice_piecerates.invoice_id = @invoiceid 
       order by t_invoice_piecerates.prd_date asc,
                t_invoice_piecerates.prt_code asc
end