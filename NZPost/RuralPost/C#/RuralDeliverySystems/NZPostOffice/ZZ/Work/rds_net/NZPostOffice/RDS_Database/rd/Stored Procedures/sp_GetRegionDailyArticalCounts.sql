...
CREATE procedure [rd].[sp_GetRegionDailyArticalCounts](
    @in_Region int,
    @in_RenewalGroup int,
    @in_Period datetime)
AS
BEGIN
  ...
  -- Get daily article counts for a all contracts in a region for data entry
  ...
  select acd.contract_no,
   ...
   where o.region_id = @in_Region 
     and c.rg_code   = @in_RenewalGroup 
