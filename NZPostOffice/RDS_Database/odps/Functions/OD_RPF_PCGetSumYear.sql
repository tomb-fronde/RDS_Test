CREATE function [odps].[OD_RPF_PCGetSumYear](
	@con_supplier int,
	@contract int,
	@con_seq_no int,
	@sdate datetime,
...
BEGIN
  ...
  -- 
  -- MB  May-2010
  -- Changes to "@NZPostEmp is null" section to improve performance.
...
--      select @amt1=iSnull(@amt1,0) - iSnull(@amt2,0)
	...
	select @amt1 = sum(payment_component.pc_amount) 
         ...
         where payment.contractor_supplier_no = @con_supplier and
               payment.contract_no = @contract and
               payment.contract_seq_number = @con_seq_no and
               payment.invoice_id = payment_component.invoice_id and
               payment_component.pct_id = payment_component_type.pct_id and
               ...
               ((payment_component.misc_string not like '%NZH%' 
                 AND payment_component.misc_string not like '%Guardian%')
				 OR payment_component.misc_string is NULL)
      ...
      begin
        ...
        select @amt2 = sum(payment_component.pc_amount) 
         ...
         where payment.contractor_supplier_no = @con_supplier and
               ...
               contractor.nz_post_employee = 'Y' and
               payment.invoice_date between @sdate and @edate and
               (payment_component.misc_string like '%NZH%' 
                or payment_component.misc_string like '%Guardian%')
      ...
      begin
        ...
        select @amt2 = sum(payment_component.pc_amount) 
         ...
         where payment.contractor_supplier_no = @con_supplier and
               ...
               payment_component_type.pcg_id = payment_component_group.pcg_id and
               payment_component_group.pcg_short_code = @ShortCode and
               (contractor.nz_post_employee = 'N' 
                or contractor.nz_post_employee is null) and
               payment.invoice_date between @sdate and @edate and
               (payment_component.misc_string like '%NZH%' 
                or payment_component.misc_string like '%Guardian%')
