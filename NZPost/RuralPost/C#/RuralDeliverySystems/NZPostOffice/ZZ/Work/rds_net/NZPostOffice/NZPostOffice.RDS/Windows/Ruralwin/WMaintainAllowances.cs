...
namespace NZPostOffice.RDS.Windows.Ruralwin
{
    ...
    public class WMaintainAllowances : WAncestorWindow
    {
        ...
        public WMaintainAllowances()
        {
            ...
            this.dw_allowance.DataObject = new DMaintainAllowance();
        ...
        private void InitializeComponent()
        {
            ...
            // st_label
            // 
            st_label.Text = "WMaintainAllowances";
            st_label.Width = 136;
        ...
        public override void pfc_postopen()
        {
            base.pfc_postopen();
            int ll_row, nAlctId;
            string ls_title, sAlctDescription;
            ...
            dEffectiveDate = System.Convert.ToDateTime(lvn_Criteria.of_getcriteria("effective_date"));
            nAlctId = System.Convert.ToInt32(lvn_Criteria.of_getcriteria("alct_id"));
            sAlctDescription = lvn_Criteria.of_getcriteria("alct_description") as string;

            idw_allowance.DataObject.Reset();
            ((DMaintainAllowance)idw_allowance.DataObject).Retrieve(il_contract, dEffectiveDate, nAlctId);
            /*
                        idw_allowance.DataObject.AddItem<MaintainAllowance>(new MaintainAllowance());
                        ll_row = idw_allowance.GetRow();//.RowCount;
                        idw_allowance.GetItem<MaintainAllowance>(ll_row).ContractTitle = ls_title;
                        idw_allowance.GetItem<MaintainAllowance>(ll_row).ContractNo = il_contract;
                        idw_allowance.GetItem<MaintainAllowance>(ll_row).AltKey = il_altKey;
                        idw_allowance.GetItem<MaintainAllowance>(ll_row).AlctId = 1;
            */
        ...
        public virtual int wf_validate(int arow)
        {
            ...
            int FAILURE2 = -(2);
            is_errmsg = "";
            ld_effdate = idw_allowance.GetItem<MaintainAllowance>(arow).EffectiveDate;
            ldc_amount = idw_allowance.GetItem<MaintainAllowance>(arow).AnnualAmount;
            ll_altkey = idw_allowance.GetItem<MaintainAllowance>(arow).AltKey;
        ...
        public virtual void cb_save_clicked(object sender, EventArgs e)
        {
            ...
            idw_allowance.Save();
            if (idw_allowance.GetItem<MaintainAllowance>(0).SQLCode != 0)
            {
                MessageBox.Show("Unable to update.  \n\n" 
                               + "Error Code: " + idw_allowance.GetItem<MaintainAllowance>(0).SQLCode.ToString() + "\n\n" 
                               + "Error Text: " + idw_allowance.GetItem<MaintainAllowance>(0).SQLErrText
                               , "Database Error"
                               , MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
