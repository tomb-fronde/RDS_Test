/****** Object:  StoredProcedure [rd].[sp_cust_list_town]    Script Date: 08/05/2008 10:18:15 ******/
SET ANSI_NULLS ON
GO
...
begin
  ...
  declare recipient_list cursor local scroll dynamic for 
            select recip_num, recip_char_b
              from #recipient_temp
  ...
  insert into #recipient_temp(recip_num,recip_char_b)
    select master_cust_id,
           ', '+rd.trim(isnull(cust_surname_company,'')+' '+isnull(cust_initials,'')) 
      from rds_customer
     where master_cust_id = any(select master_num from #master_temp) 
       and (cust_surname_company is not null or cust_initials is not null) 
       and (len(rtrim(cust_surname_company)) > 0 or len(rtrim(cust_initials)) > 0)
  ...
  -- update #master_temp 
  --    set #master_temp.recip_char = ISNULL(#master_temp.recip_char,'')+#recipient_temp.recip_char_b 
  --   from #recipient_temp 
  ...
  begin
    ...
    -- get the current recipient name and customer number
    ...
    -- Append this recipient to the recipient list
  ...
  insert into #occ_temp(sur_comp_name,
    ...
    select cmb.cmb_cust_surname,
           ...
           0, -- Resident
           0, -- Farmer 
           (case when (cmb_cust_surname is null or cmb_cust_surname = '') 
                  and (cmb_cust_initials is null or cmb_cust_initials = '') 
                 then  null else 1 end) as kiwimail_qty,
           row_number() over (order by cmb.adr_rd_no asc) -- cmb_seq
      from cmb_address as cmb ,
           towncity as tc 
     where cmb.tc_id = @town_id and
           cmb.tc_id = tc.tc_id 
