...
namespace NZPostOffice.RDS.Entity.Ruralwin
{
	...
	public class RestoreCusts : Entity<RestoreCusts>
	{
		...
		{
			return string.Format( "{0}/{1}/{2}", _adr_id,_cust_id,_move_in_date );
		}
		#endregion

		#region Factory Methods
		public static RestoreCusts NewRestoreCusts( int? in_addr_id )
		{
			return Create(in_addr_id);
		}

		public static RestoreCusts[] GetAllRestoreCusts( int? in_addr_id )
		{
			return Fetch(in_addr_id).list;
		}
		#endregion

		#region Data Access
		[ServerMethod]
		private void FetchEntity( int? in_addr_id )
		{
			using ( DbConnection cn= DbConnectionFactory.RequestNextAvaliableSessionDbConnection( "NZPO"))
			{
				using (DbCommand cm = cn.CreateCommand())
				{
					...
					pList.Add(cm, "in_addr_id", in_addr_id);
					cm.CommandText =" SELECT customer_address_moves.adr_id,customer_address_moves.cust_id,"+   
                        "customer_address_moves.move_out_date,rds_customer.cust_title,rds_customer.cust_surname_company,"+   
                        "rds_customer.cust_initials,rds_customer.master_cust_id,customer_address_moves.move_out_source,"+   
                        ...
                        "( customer_address_moves.adr_id = @in_addr_id ) AND  "+
					...
					List<RestoreCusts> _list = new List<RestoreCusts>();
					using (MDbDataReader dr = DBHelper.ExecuteReader(cm, pList))
					{
						while (dr.Read())
						{
							RestoreCusts instance = new RestoreCusts();
		...
		[ServerMethod()]
		private void CreateEntity( int? adr_id, int? cust_id, DateTime? move_in_date )
		{
			_adr_id = adr_id;
			_cust_id = cust_id;
