/****** Object:  StoredProcedure [rd].[rds_npad_create_address]    Script Date: 08/05/2008 10:17:36 ******/
SET ANSI_NULLS ON
GO
  ...
  if @status = 0
    begin
      ...
      if @@error <> 0
        begin
          select @status=-1
          select @description='SQL Error '+convert(varchar(6),@@error)+' counting contract#s in post_code'
        end
        
      if @temp_int = 1 and @status = 0
        begin
          select distinct @contract_no = contract_no 
            from address join post_code 
                           on address.post_code_id = post_code.post_code_id 
                         join road 
                           on address.road_id = road.road_id 
          ...
          if @@error <> 0
            begin
              select @status=-1
              select @description='SQL Error '+convert(varchar(6),@@error)+' getting contract# for address'
            end
        end
      else
      ...
      if @numbered = 1 and @status = 0
        begin
          ...
          -------------------------------------
          insert into address (adr_id, adr_no, adr_unit, adr_alpha, adr_property_identification,
                               road_id, sl_id, tc_id, adr_rd_no, post_code_id,
                               dp_id, contract_no, adr_date_loaded, adr_last_amended_user) 
          values (@adrID, @adrNo, @adrUnit, @adrAlpha, @adrProperty,
        ...
        begin
          ...
          if @masterDpid is null
            begin
              ...
              values (@adrID, null, null, null, @adrProperty,
              ...
              if @status = 0
                begin
                  insert into rds_customer (cust_id, cust_title, cust_initials, cust_surname_company,
                  ...
                  values (@custID, @custTitle, @custInitials, @tmp_in_cust_surname,
                          null, 'Y', 'N', 'Y', 'N',
