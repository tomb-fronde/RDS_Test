...
namespace NZPostOffice.RDS.Entity.Ruralrpt
{
	...
	public class PsCustListAny : Entity<PsCustListAny>
	{
		...
		public virtual int? SeqNo
		{
			get
			{
                CanReadProperty("SeqNo", true);
				return _seq_no;
			}
			set
			{
                CanWriteProperty("SeqNo", true);
				if ( _seq_no != value )
				{
					_seq_no = value;
					PropertyHasChanged();
...
//?			if( trim(road_unit) = '','', trim(road_unit)+'/') + trim(road_no) + if(isnull(road_alpha),'  ', road_alpha)
		...
		public static PsCustListAny NewPsCustListAny(  )
		{
			return Create();
		}

		public static PsCustListAny[] GetAllPsCustListAny(  )
		{
			return Fetch().list;
		}
		#endregion
		...
		private void FetchEntity(  )
		{
			using ( DbConnection cn= DbConnectionFactory.RequestNextAvaliableSessionDbConnection( "NZPO"))
			{
				using (DbCommand cm = cn.CreateCommand())
				{
                    ...
                    cm.CommandText = "rd.sp_cust_list_any";
					ParameterCollection pList = new ParameterCollection();

					List<PsCustListAny> _list = new List<PsCustListAny>();
					using (MDbDataReader dr = DBHelper.ExecuteReader(cm, pList))
					{
						while (dr.Read())
						{
							PsCustListAny instance = new PsCustListAny();
                            instance._seq_no = GetValueFromReader<int?>(dr,0);
                            instance._sur_comp_name = GetValueFromReader<string>(dr,1);
                            instance._initials = GetValueFromReader<string>(dr,2);
                            instance._prop_title = GetValueFromReader<string>(dr,3);
                            ...
                            instance._road_alpha = GetValueFromReader<string>(dr,16);
                            instance._road_unit = GetValueFromReader<string>(dr,17);
							instance.MarkOld();
