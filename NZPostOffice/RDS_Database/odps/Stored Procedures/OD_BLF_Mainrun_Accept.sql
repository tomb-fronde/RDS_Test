...
CREATE procedure [odps].[OD_BLF_Mainrun_Accept] (
	@enddat DATETIME,
	@SWP_Re INT      OUTPUT )
AS
BEGIN
    -- TJB  Agresso AP File format  Jan-2014: Bug fix
    -- Added column names to inserts into odps.payment 
    -- (batch_no added but not in select lists).
    -- Fixed commit (removed "work")
    --
    -- TJB  RPCR_021  Sept-2010
    -- Add alt_key to payment_component inserts from t_payment_component
    -- (this was populated in od_blf_mainrun_grosspay)
...
BEGIN TRANSACTION
      ...
      update odps.t_payment_component 
         set pc_amount = odps.t_payment_component.pc_amount*-1  
       where odps.t_payment_component.pct_id 
                 = (select max(odps.Payment_Component_Type.pct_id) 
                      from odps.payment_component_group,
                           odps.Payment_Component_Type 
                     where odps.Payment_Component_Type.pcg_id = odps.payment_component_group.pcg_id and
  ...
  -- TWC - 15/09/2003 - check date against contractor tenure dates.
      update rd.contract_adjustments 
         set ca_date_paid = @enddat  
       where exists(select odps.t_payment.contract_no 
                      from odps.t_payment 
                     where odps.t_payment.contract_no = rd.contract_adjustments.contract_no and
                           rd.contract_adjustments.ca_date_occured 
                                    <= rd.getContractorEnd(odps.t_payment.contract_no,odps.t_payment.contractor_supplier_no) and 
                           rd.contract_adjustments.ca_date_occured 
                                    >= rd.getContractorStart(odps.t_payment.contract_no,odps.t_payment.contractor_supplier_no)) and 
      ...
      update rd.frequency_adjustments 
         set fd_paid_to_date = @enddat  
       where exists(select odps.t_payment.contract_no 
                      from odps.t_payment 
                     where odps.t_payment.contract_no = rd.frequency_adjustments.contract_no and
      ...
      update rd.contract_allowance 
         set ca_paid_to_date = @enddat  
       where exists(select odps.t_payment.contract_no 
                      from odps.t_payment 
                     where odps.t_payment.contract_no = rd.contract_allowance.contract_no) 
         and ca_effective_date <= (select max(invoice_date) 
                                     from odps.t_payment) 
      ...
      /*Cannot update identity column 'pr_id' in MSSQL */
		...
		insert into odps.t_payment_runs_2 
           (pr_id, pr_date, gl_posted, pr_ap_posted, pr_contract_no, POTS)
           select pr_id, pr_date, gl_posted, pr_ap_posted, pr_contract_no, POTS
             from odps.t_payment_runs 
		update odps.t_payment_runs_2 
		   set pr_id = (select isnull(max(pr_id),0)+1 from odps.payment_runs)
      ...
      insert into odps.payment_runs 
          (pr_id,pr_date,gl_posted,pr_ap_posted,pr_contract_no,POTS)
        select (select isnull(max(pr_id),0)+1 from odps.payment_runs),
               pr_date,
               gl_posted,
               pr_ap_posted,
               pr_contract_no,
      ...
      begin
         insert into odps.Payment 
            (contractor_supplier_no, contract_no, contract_seq_number, Invoice_ID, 
             pr_id, invoice_date, Witholding_tax_rate_applied, invoice_no,
             case_no, POTS )
         ...
         insert into odps.post_tax_deductions_applied
      ...
      begin
         select @v_max_invoice_no = max(invoice_no)  
           from odps.Payment 
          where invoice_date = @enddat
         ...
         select @v_max_invoice_id = max(invoice_id) 
           from odps.Payment
         ...
         select contractor_supplier_no,
                contract_no,
                contract_seq_number,
                invoice_id + @v_max_invoice_id,
                pr_id,
                invoice_date,
                Witholding_tax_rate_applied,
                (ROW_NUMBER() OVER (ORDER BY contract_no ASC)) + @v_max_invoice_no,
                case_no,
                POTS 
           from odps.t_payment
      ...
      select @v_payment_component_count = 1   
       where exists(select 1 from odps.Payment_Component)
      ...
      begin
         insert into odps.Payment_Component 
             (pc_id, pct_id, invoice_id, pc_amount, comments, 
              misc_date, misc_string, misc_decimal, alt_key) 
           select pc_id, pct_id, invoice_id, pc_amount, comments, 
                  misc_date, misc_string, misc_decimal, alt_key 
      ...
      begin
         select @v_max_pc_id = max(pc_id) + 1 
           from odps.Payment_Component
...
--         end
		...
		insert into odps.t_payment_component_2 
		    (pc_id,  pct_id, invoice_id, pc_amount, comments, 
             misc_date, misc_string, misc_decimal, alt_key)
		select pc_id + @v_max_pc_id ,
		       pct_id,
		       invoice_id,
		       pc_amount,
		       comments,
		...
		insert into odps.t_payment_component
		  ...
		  select pc_id, pct_id, invoice_id, pc_amount, comments, 
                 misc_date, misc_string, misc_decimal, alt_key
		    from odps.t_payment_component_2
      ...
      select @v_payment_pr_count = 1 
       where exists(select 1 from odps.Payment_component_piece_rates)
      ...
      begin
         ...
         begin  --added
            insert into odps.Payment_component_piece_rates
                (prt_key,pc_id,pcpr_volume,pcpr_value) 
            select prt_key,
                   pc_id + @v_max_pc_id,
                   pcpr_volume,
                   pcpr_value 
              from odps.t_payment_component_piece_rates
      ...
      begin
         select @v_max_pcpr_id = max(pcpr_id) + 1 
		...
		insert into odps.t_payment_component_piece_rates 
            (pcpr_id,prt_key,pc_id,pcpr_volume,pcpr_value) 
		  select pcpr_id,prt_key,pc_id,pcpr_volume,pcpr_value
            from odps.t_payment_component_piece_rates_2 
	    ...
	    ---end changed
         ...
         insert into odps.Payment_component_piece_rates
             (prt_key,pc_id,pcpr_volume,pcpr_value) 
         select prt_key, pc_id, pcpr_volume, pcpr_value 
           from odps.t_payment_component_piece_rates
  ...
  -- TWC - 15/09/2003 check prd_date against contractors tenure dates
      update rd.piece_rate_delivery 
         set prd_paid_to_date = @enddat  
       where exists(select odps.Payment.contract_no 
                      from odps.Payment 
                     where odps.Payment.contract_no = rd.piece_rate_delivery.contract_no and
                           rd.piece_rate_delivery.prd_date 
                               <= rd.getContractorEnd(odps.Payment.contract_no,odps.Payment.contractor_supplier_no) and -- end date
                           rd.piece_rate_delivery.prd_date 
                               >= rd.getContractorStart(odps.Payment.contract_no,odps.Payment.contractor_supplier_no)) and -- start date
                           prd_paid_to_date is null and
                           prd_date between (select min(sdate) from odps.t_piecerate_tracker 
                                              where odps.t_piecerate_tracker.contract_no = rd.piece_rate_delivery.contract_no)
                                        and (select max(edate) from odps.t_piecerate_tracker 
                                              where odps.t_piecerate_tracker.contract_no = rd.piece_rate_delivery.contract_no)
