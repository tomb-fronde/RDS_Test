...
namespace NZPostOffice.ODPS.Entity.OdpsInvoice
{
    ...
    public class InvoiceDetailPaymentPrDetailcp : Entity<InvoiceDetailPaymentPrDetailcp>
    {
        ...
        public virtual int? PrdQuantity
        {
            get
            {
                CanReadProperty("CanReadProperty",true);
                return _prd_quantity;
            }
            set
            {
                CanWriteProperty("CanReadProperty",true);
        ...
        public static InvoiceDetailPaymentPrDetailcp NewInvoiceDetailPaymentPrDetailcp(int? invoiceid, int? contractno, int? contractorno, DateTime? payperiod_start, DateTime? payperiod_end)
        {
            return Create(invoiceid, contractno, contractorno, payperiod_start, payperiod_end);
        }

        public static InvoiceDetailPaymentPrDetailcp[] GetAllInvoiceDetailPaymentPrDetailcp(int? invoiceid, int? contractno, int? contractorno, DateTime? payperiod_start, DateTime? payperiod_end)
        {
        ...
        private void FetchEntity(int? invoiceid, int? contractno, int? contractorno, DateTime? payperiod_start, DateTime? payperiod_end)
        {
            using (DbConnection cn = DbConnectionFactory.RequestNextAvaliableSessionDbConnection("NZPO"))
            {
                using (DbCommand cm = cn.CreateCommand())
                {
                    ...
                    cm.CommandText = "odps.od_rps_invoice_pay_piecerate_detailcp";
                    ...
                    List<InvoiceDetailPaymentPrDetailcp> _list = new List<InvoiceDetailPaymentPrDetailcp>();
                    using (MDbDataReader dr = DBHelper.ExecuteReader(cm, pList))
                    {
                        while (dr.Read())
                        {
                            InvoiceDetailPaymentPrDetailcp instance = new InvoiceDetailPaymentPrDetailcp();
                        ...
                        //if (dr.Read())
                        //{
                        //    InvoiceDetailPaymentPrDetailcp item = new InvoiceDetailPaymentPrDetailcp();
                        //    _list.Add(item);
                        //}
                        list = _list.ToArray();
                    }
