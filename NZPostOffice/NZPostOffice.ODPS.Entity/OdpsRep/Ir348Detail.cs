...
namespace NZPostOffice.ODPS.Entity.OdpsRep
{
    ...
    public class Ir348Detail : Entity<Ir348Detail>
    {
        ...
        public static Ir348Detail NewIr348Detail(DateTime? startdate, DateTime? enddate)
        {
            return Create(startdate, enddate);
        }

        public static Ir348Detail[] GetAllIr348Detail(DateTime? startdate, DateTime? enddate)
        {
        ...
        private void FetchEntity(DateTime? startdate, DateTime? enddate)
        {
            using (DbConnection cn = DbConnectionFactory.RequestNextAvaliableSessionDbConnection("NZPO"))
            {
                using (DbCommand cm = cn.CreateCommand())
                {
                    ...
                    cm.CommandText = "odps.od_rps_ir348detail";
                    ParameterCollection pList = new ParameterCollection();
                    pList.Add(cm, "startdate", startdate);
                    pList.Add(cm, "enddate", enddate);

                    List<Ir348Detail> _list = new List<Ir348Detail>();
                    using (MDbDataReader dr = DBHelper.ExecuteReader(cm, pList))
                    {
                        while (dr.Read())
                        {
                            Ir348Detail instance = new Ir348Detail();
