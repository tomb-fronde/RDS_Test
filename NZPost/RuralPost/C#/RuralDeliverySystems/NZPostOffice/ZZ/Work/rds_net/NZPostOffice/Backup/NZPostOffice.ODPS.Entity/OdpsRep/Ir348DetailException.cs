...
namespace NZPostOffice.ODPS.Entity.OdpsRep
{
    ...
    public class Ir348DetailException : Entity<Ir348DetailException>
    {
        ...
        public static Ir348DetailException NewIr348DetailException(DateTime? startdate, DateTime? enddate)
        {
            return Create(startdate, enddate);
        }

        public static Ir348DetailException[] GetAllIr348DetailException(DateTime? startdate, DateTime? enddate)
        {
        ...
        private void FetchEntity(DateTime? startdate, DateTime? enddate)
        {
            using (DbConnection cn = DbConnectionFactory.RequestNextAvaliableSessionDbConnection("NZPO"))
            {
                using (DbCommand cm = cn.CreateCommand())
                {
                    ...
                    List<Ir348DetailException> _list = new List<Ir348DetailException>();
                    using (MDbDataReader dr = DBHelper.ExecuteReader(cm, pList))
                    {
                        while (dr.Read())
                        {
                            Ir348DetailException instance = new Ir348DetailException();
