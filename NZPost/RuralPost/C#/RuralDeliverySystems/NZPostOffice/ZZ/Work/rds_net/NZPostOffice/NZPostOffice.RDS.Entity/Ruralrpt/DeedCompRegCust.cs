...
namespace NZPostOffice.RDS.Entity.Ruralrpt
{
    ...
    public class DeedCompRegCust : Entity<DeedCompRegCust>
    {
        ...
        public virtual DateTime? Compute1
        {
            get
            {
                CanReadProperty(true);
        ...
        public static DeedCompRegCust NewDeedCompRegCust()
        {
            return Create();
        }

        public static DeedCompRegCust[] GetAllDeedCompRegCust()
        {
            return Fetch().list;
        }
        #endregion
        ...
        private void FetchEntity()
        {
            using (DbConnection cn = DbConnectionFactory.RequestNextAvaliableSessionDbConnection("NZPO"))
            {
                using (DbCommand cm = cn.CreateCommand())
                {
                    ...
                    cm.CommandText = "rd.sp_deed_compliance_cust";
                    ParameterCollection pList = new ParameterCollection();

                    List<DeedCompRegCust> _list = new List<DeedCompRegCust>();
                    using (MDbDataReader dr = DBHelper.ExecuteReader(cm, pList))
                    {
                        while (dr.Read())
                        {
                            DeedCompRegCust instance = new DeedCompRegCust();
