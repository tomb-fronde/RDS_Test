			...
			using ( DbConnection cn= DbConnectionFactory.RequestNextAvaliableSessionDbConnection( "NZPO"))
			{
				using (DbCommand cm = cn.CreateCommand())
				{
                    ...
                    cm.CommandText = " SELECT address.adr_id as adr_id,"
                        ...
                        + " max(case when cust_initials is null then rds_customer.cust_surname_company else rds_customer.cust_surname_company +', '+cust_initials end) as customer,"
                        ...
                        + " FROM address INNER JOIN address_frequency_sequence"
                        + "      ON address.adr_id = address_frequency_sequence.adr_id AND"
                        + "      address_frequency_sequence.rf_delivery_days = @as_rf_delivery_days AND"
                        + "      address_frequency_sequence.sf_key           = @al_sf_key"
                        ...
                        + "   ON customer_address_moves.adr_id = address.adr_id AND"
                        + "      customer_address_moves.move_out_date is null AND"
                        + "     (customer_address_moves.move_in_date is null"
                        ...
                        + "   ON rds_customer.cust_id = customer_address_moves.cust_id AND"
                        + "      rds_customer.master_cust_id is null"
                        + " WHERE address.contract_no = @al_contract_no"
                        + " GROUP BY address_frequency_sequence.contract_no, address.adr_id, adr_unit, adr_no, adr_alpha," 
                        + "   road.road_name + case when road_type.rt_name is null then '' else ' '+road_type.rt_name end"
