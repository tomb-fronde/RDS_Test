...
namespace NZPostOffice.ODPS.Entity.OdpsInvoice
{
    ...
    public class InvoiceDetailPaymentPrDetailpp : Entity<InvoiceDetailPaymentPrDetailpp>
    {
        ...
        public static InvoiceDetailPaymentPrDetailpp NewInvoiceDetailPaymentPrDetailpp(int? invoiceid, int? contractno, int? contractorno, DateTime? payperiod_start, DateTime? payperiod_end)
        {
            return Create(invoiceid, contractno, contractorno, payperiod_start, payperiod_end);
        }

        public static InvoiceDetailPaymentPrDetailpp[] GetAllInvoiceDetailPaymentPrDetailpp(int? invoiceid, int? contractno, int? contractorno, DateTime? payperiod_start, DateTime? payperiod_end)
        {
        ...
        private void FetchEntity(int? invoiceid, int? contractno, int? contractorno, DateTime? payperiod_start, DateTime? payperiod_end)
        {
            using (DbConnection cn = DbConnectionFactory.RequestNextAvaliableSessionDbConnection("NZPO"))
            {
                using (DbCommand cm = cn.CreateCommand())
                {
                    ...
                    cm.CommandText = "odps.od_rps_invoice_pay_piecerate_detailpp";
                    ...
                    List<InvoiceDetailPaymentPrDetailpp> _list = new List<InvoiceDetailPaymentPrDetailpp>();
                    using (MDbDataReader dr = DBHelper.ExecuteReader(cm, pList))
                    {
                        while (dr.Read())
                        {
                            InvoiceDetailPaymentPrDetailpp instance = new InvoiceDetailPaymentPrDetailpp();
                        ...
                        //if (dr.Read())
                        //{
                        //    InvoiceDetailPaymentPrDetailpp item = new InvoiceDetailPaymentPrDetailpp();
                        //    _list.Add(item);
                        //}
                        list = _list.ToArray();
                    }
